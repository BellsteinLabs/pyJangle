...
def RegisterSaga(name: str = None):
    ...
    def decorator(cls):
        ...
        if saga_name in __name_to_saga_type_map:
            raise SagaRegistrationError("Name already registered", {"name": saga_name, "current_registrant": str(__name_to_saga_type_map[saga_name]), "duplicate_registrant": str(cls)})
        __name_to_saga_type_map[saga_name] = cls
        __saga_type_to_name_map[cls] = saga_name
        log(LogToggles.saga_registered, "Saga registered", {"saga_name": saga_name, "saga_type": str(cls)})
        return cls
...
def get_saga_type(name: str) -> type:
    ...
    try:
        ...
        raise KeyError(f"No saga registered with name: {name}.  Ensure the saga is decorated with @RegisterSaga.")
...
def get_saga_name(saga_type: type) -> str:
    ...
    try:
        ...
        raise KeyError(f"{str(type)} is not registered as an saga.  Ensure the saga is decorated with @RegisterSaga.")
