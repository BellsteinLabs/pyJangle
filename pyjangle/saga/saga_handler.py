...
def handle_saga_event(saga_id: any, event: Event, saga_type: type[Saga]):
    ...
    log(LogToggles.saga_retrieved, "Retrieved saga", {"id": saga_id, "metadata": saga_metadata.__dict__, "events": [x.__dict__ for x in saga_events]})
    ...
    if event: 
        log(LogToggles.apply_event_to_saga, "Applying event to saga", {"id": saga_id, "saga_type": str(type(saga)), "saga": saga.__dict__,"event": event.__dict__})
