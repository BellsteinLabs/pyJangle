...
from pyjangle.event.event_dispatcher import enqueue_committed_event_for_dispatch, event_dispatcher_instance
...
async def handle_command(command: Command) -> CommandResponse:
    ...
    log(LogToggles.command_received, "Command received", {"aggregate_id": aggregate_id, "command_data": command.__dict__})
    while True:
        ...
        log(LogToggles.aggregate_created, "Blank aggregate created", {"aggregate_id": aggregate_id, "aggregate_type": str(type(aggregate))})
        ...
        log(LogToggles.retrieved_aggregate_events, "Retrieved aggregate events", {"aggregate_id": aggregate_id, "aggregate_type": str(type(aggregate)), "event_count": len(list(events))})
        ...
        if command_response.is_success:
            try:
                ...
                for e in aggregate.new_events:
                    log(LogToggles.committed_event, "Event committed", {"aggregate_id": aggregate_id, "event": e.__dict__})
...
async def _apply_snapshotting_to_aggregate(aggregate: Snapshottable, command: Command) -> Aggregate:
    ...
    if not is_snapshotting:
        return aggregate
    log(LogToggles.is_snapshotting, f"Snapshotting status", {"enabled": is_snapshotting, "aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate))})
    ...
    log(LogToggles.is_snapshot_found, f"Snapshot was {'found' if version_and_snapshot_tuple != None else 'not found'}.", {"aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate)), "version": version_and_snapshot_tuple[0] if version_and_snapshot_tuple != None else None})
    if version_and_snapshot_tuple != None:
        ...
        try:
            ...
            log(LogToggles.snapshot_applied, f"Snapshot applied", {"aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate)), "version": version_and_snapshot_tuple[0]})
        except Exception as e:
            #Code change in the aggregate probably caused this snapshot 
            #to become outdated, most likely.  The snapshot will be 
            #deleted.
            log(LogToggles.snapshot_application_failed, "Snapshot application failed", exc_info=e)
            ...
            log(LogToggles.snapshot_deleted, "Deleted snapshot", {"aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate))})
            return aggregate
    return aggregate
...
async def _record_new_snapshot_if_applicable(aggregate_id: any, aggregate: Aggregate):
    ...
    if updated_version % snapshotable.get_snapshot_frequency() == 0:
        ...
        log(LogToggles.snapshot_taken, "Snapshot recorded", {"aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate)), "version": updated_version})
    else: 
        log(LogToggles.snapshot_not_needed, "Snapshot not needed",{"aggregate_id": aggregate.id, "aggregate_type": str(type(aggregate)), "version": updated_version})
...
async def _dispatch_events_locally(events: List[Event]):
    ...
    for event in events: 
        ...
        log(LogToggles.dispatched_event_locally, "Events dispatched locally", {"events": [{"event_type": str(type(e)), "event_data": e.__dict__} for e in events]})
