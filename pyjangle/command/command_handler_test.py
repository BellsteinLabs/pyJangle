...
import pyjangle
from pyjangle import handle_command

from pyjangle import (DuplicateKeyError,
                      event_repository_instance)
from pyjangle.test.commands import (AnotherCommandThatAlwaysSucceeds,
                                    CommandThatAlwaysSucceeds,
                                    CommandThatFails)
...
from pyjangle.test.transient_snapshot_repository import \
    TransientSnapshotRepository
...
class TestCommandHandler(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_after_duplicate_key_exception(self, _, event_repo, __):
        def dupliate_key_error_on_2nd_call_side_effect(real_function, *args):
            dupliate_key_error_on_2nd_call_side_effect.counter = 0 if not hasattr(
                dupliate_key_error_on_2nd_call_side_effect, "counter") else dupliate_key_error_on_2nd_call_side_effect.counter
        ...
        event_repo.commit_events = Mock(
            side_effect=lambda *args: dupliate_key_error_on_2nd_call_side_effect(real_func, *args))
