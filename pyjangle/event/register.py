...
def RegisterEvent(name: str = None):
    ...
    def decorator(cls):
        ...
        if event_name in __name_to_event_type_map:
            raise EventRegistrationError("Name already registered", {"name": event_name, "current_registrant": str(__name_to_event_type_map[event_name]), "duplicate_registrant": str(cls)})
        __name_to_event_type_map[event_name] = cls
        __event_type_to_name_map[cls] = event_name
        log(LogToggles.event_registered, "Event registered", {"event_name": event_name, "event_type": str(cls)})
        return cls
...
def get_event_type(name: str) -> type:
    ...
    try:
        ...
        raise KeyError(f"No event registered with name: {name}.  Ensure the event is decorated with @RegisterEvent.")
