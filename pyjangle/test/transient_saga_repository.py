...
class TransientSagaRepository(SagaRepository):
    ...
    async def get_saga(self, saga_id: any) -> tuple[SagaMetadata, List[SagaEvent]]:
    ...
    async def commit_saga(self, metadata: SagaMetadata, events: list[Event | SagaEvent]):
        ...
        if set([event.id for event in events if not issubclass(type(event), SagaEvent)]).intersection(set([event.id for event in existing_events if not issubclass(type(event), SagaEvent)])):
            raise DuplicateKeyError()
        if set([event.version for event in events if issubclass(type(event), SagaEvent)]).intersection(set([event.version for event in existing_events if not issubclass(type(event), SagaEvent)])):
            raise DuplicateKeyError()
    ...
    async def get_retry_saga_metadata(self, max_count: int) -> list[SagaMetadata]:
        ...
        return [metadata for metadata in self.metadata.values() if not metadata.is_complete and (not metadata.timeout_at or metadata.timeout_at > current_time) and (metadata.retry_at and metadata.retry_at < current_time)]