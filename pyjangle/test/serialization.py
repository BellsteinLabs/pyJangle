...
def deserialize_saga(serialized: tuple[dict, list[dict]]) -> Saga:
    ...
    saga_retry_at = datetime.fromisoformat(metadata_dict[FIELDS.SAGA_METADATA.RETRY_AT]) if metadata_dict[FIELDS.SAGA_METADATA.RETRY_AT] else None
    saga_timeout_at = datetime.fromisoformat(metadata_dict[FIELDS.SAGA_METADATA.TIMEOUT_AT]) if metadata_dict[FIELDS.SAGA_METADATA.TIMEOUT_AT] else None
    ...
    for serialized_event in serialized[1]:
        ...
        event_class_instance: type[Event] = get_event_type(serialized_event[FIELDS.EVENT_STORE.TYPE])
