...
ACCOUNT_CREATION_AGGREGATE_ID = uuid.uuid3(uuid.NAMESPACE_URL, "ACCOUNT_CREATION_AGGREGATE")
...
class AccountCreationAggregate(Aggregate):
    ...
    def handle_create_account_command(self, command: CreateAccount, next_version: int) -> command_response:
        ...
        account_id_incremented_event = AccountIdProvisioned(version=next_version, id=ACCOUNT_CREATION_AGGREGATE_ID, created_at=datetime.now())
        self._post_new_event(account_id_incremented_event)
        account_created_event = AccountCreated(version=1, account_id=next_account_id, name=command.name, created_at=datetime.now())
