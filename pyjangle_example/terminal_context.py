...
from pyjangle_example.queries import (
    AccountLedger,
    AccountSummary,
    BankStats,
    AccountsList,
...
class InputSpec:
    def __init__(
        self,
        prompt_maker: str | Callable[[any], str],
        validator: Callable[[str], str | None],
        converter: Callable[[str], any] = None,
    ):
        self.prompt_maker = (
            prompt_maker
            if not isinstance(prompt_maker, str)
            else lambda x: prompt_maker
        )
...
class TerminalContext(metaclass=abc.ABCMeta):
    ...
    async def get_input(self):
        for i, spec in enumerate(self.input_spec):
            ...
            while not field_value:
                ...
                field_value = await get_event_loop().run_in_executor(
                    None, lambda: input(spec.prompt_maker(self)).strip()
                )
                ...
                if validation_result:
                    ...
                    continue
                converted = (
                    spec.converter(field_value) if spec.converter else field_value
                )
    ...
    async def enter_to_continue(self):
        await get_event_loop().run_in_executor(
            None, lambda: input("Enter to continue...")
        )
...
class CreateAccountContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            InputSpec("Account name (5-15 chars):", self.validate_account_name),
            InputSpec(
                "Starting balance (ENTER for 0):",
                self.validate_initial_balance,
                self.convert_initial_balance,
            ),
        ]
...
class DeleteAccountContext(TerminalContext):
    @property
    def input_spec(self) -> list[InputSpec]:
        return [InputSpec("ID of account to delete:", validate_account_id)]

    async def action(self):
        command = DeleteAccount(account_id=self.converted_input[0])
        result = await handle_command(command)
        if result.is_success:
            print(f"Account with id {self.converted_input[0]} successfully deleted")
        else:
...
class SelectAccountContext(TerminalContext):
    @property
    def input_spec(self) -> list[InputSpec]:
        return [InputSpec("ID of account to select:", validate_account_id)]
...
class AccountContext(TerminalContext):
    async def show_context(self):
        self.account_summary = await handle_query(
            AccountSummary(account_id=self.data[ACCOUNT_ID])
        )
        ...
        headers = [
            {
                "index": "#",
                "funded_account": "AcctID",
                "amount": "Amount",
                "timeout_at": "Expires At",
            }
        ]
        ...
        for i, row in enumerate(pending_outbound_transfers):
            print(
                "  {1!s: >{xfer_index_width}} {0[funded_account]!s: <{xfer_account_id_width}} {0[amount]!s: >0{amount_width}.2} {0[timeout_at]!s: >{expires_at_width}}".format(
                    row,
                    i if i > 0 else "",
                    xfer_index_width=xfer_index_width,
                    xfer_account_id_width=xfer_account_id_width,
                    amount_width=amount_width,
                    expires_at_width=expires_at_width,
                )
            )
    ...
    def input_spec(self):
        ...
        return [
            InputSpec(
                lambda x: "Select an option:",
                self.make_option_validator(),
                lambda x: int(x),
            )
        ]
    ...
    def get_next_context(self):
        ...
        return next_context_type(
            data=self.data if next_context_type != RootContext else None
        )
...
class DepositFundsContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            InputSpec(
                "How much to deposit?", validate_funds_amount, lambda x: Decimal(x)
            )
        ]
    ...
    async def action(self):
        result = await handle_command(
            DepositFunds(
                account_id=self.data[ACCOUNT_ID], amount=self.converted_input[0]
            )
        )
        if result.is_success:
            print(
                f"""{str(self.converted_input[0])} successfully deposited into 
...
class WithdrawFundsContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            InputSpec(
                "How much to withdraw?", validate_funds_amount, lambda x: Decimal(x)
            )
        ]
    ...
    async def action(self):
        ...
        if result.is_success:
            print(
                f"""{str(self.converted_input[0])} successfully withdrew from 
                {self.data[ACCOUNT_ID]}."""
            )
        else:
...
class TransferContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            ...
            InputSpec("Amount to send:", validate_funds_amount, lambda x: Decimal(x)),
        ]

    async def action(self):
        result = await handle_command(
            Transfer(
                funded_account_id=self.converted_input[0],
                funding_account_id=self.data[ACCOUNT_ID],
                amount=self.converted_input[1],
            )
        )
        if result.is_success:
            print(
                f"""Sent {str(self.converted_input[1])} to account ID 
                {self.converted_input[0]}."""
            )
        else:
            print(
                f"""There was an error sending funds to {self.converted_input[0]}: 
                {result.data}"""
            )
        await self.enter_to_continue()
...
class RequestContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            InputSpec("Account ID to request funds from:", validate_account_id),
            InputSpec(
                "Amount to request:", validate_funds_amount, lambda x: Decimal(x)
            ),
        ]
    ...
    async def action(self):
        result = await handle_command(
            Request(
                funded_account_id=self.data[ACCOUNT_ID],
                funding_account_id=self.converted_input[0],
                amount=self.converted_input[1],
            )
        )
        if result.is_success:
            print(
                f"""Requested {str(self.converted_input[1])} from account ID 
                {self.converted_input[0]}."""
            )
        else:
            print(
                f"""There was an error requesting funds from {self.converted_input[0]}: 
                {result.data}"""
            )
        await self.enter_to_continue()
...
class AcceptRequestContext(TerminalContext):
    ...
    def input_spec(self) -> list[InputSpec]:
        return [
            InputSpec(
                "Which entry to confirm?",
                make_index_validator(len(self.data[ACCOUNT_SUMMARY].transfer_requests)),
            )
        ]

    async def action(self):
        pending_transfer = self.data[ACCOUNT_SUMMARY].transfer_requests[
            int(self.converted_input[0]) - 1
        ]
        result = await handle_command(
            AcceptRequest(
                funding_account_id=self.data[ACCOUNT_ID],
                transaction_id=pending_transfer.transaction_id,
            )
        )
        if result.is_success:
            print(
                f"""Accepted request to transfer {pending_transfer.amount} to account ID 
                {pending_transfer.funded_account}"""
            )
        else:
            print(
                f"""There was an error accepting request to transfer funds: 
                {result.data}"""
            )
        await self.enter_to_continue()
...
class RejectRequestContext(TerminalContext):
    ...
    async def action(self):
        ...
        if result.is_success:
            print(
                f"""Rejected request to transfer {pending_transfer.amount} to account ID 
                {pending_transfer.funded_account}"""
            )
        else:
            print(
                f"""There was an error rejecting request to transfer funds: 
                {result.data}"""
            )
        await self.enter_to_continue()
...
class ViewAccountLedgerContext(TerminalContext):
    async def show_context(self):
        self.ledger = await handle_query(
            AccountLedger(account_id=self.data[ACCOUNT_ID])
        )
        display_width = os.get_terminal_size().columns
        index_width = 4
        ...
        for i, row in enumerate(transactions):
            print(
                "  {1!s: >{index_width}} {0[initiated_at]!s: >{initiated_at_width}} {0[amount]!s: >0{amount_width}.2} {0[transaction_type]!s: >{description_width}}".format(
                    row,
                    i,
                    index_width=index_width,
                    initiated_at_width=initiated_at_width,
                    amount_width=amount_width,
...
class ViewStatisticsContext(TerminalContext):
    async def show_context(self):
        ...
        for stat in stats:
            print(
                "{0[description]: >{half_display_width}}:  {0[value]: <{half_display_width}}".format(
                    stat,
                    display_width=display_width,
                    half_display_width=half_display_width,
                )
            )
...
async def show_accounts_summary():
    ...
    for row in bank_summary:
        print(
            "{0[account_id]!s: >{id_width}} {0[name]!s: <{name_width}} {0[balance]!s: >0{balance_width}.2} {0[pending_request_count]!s: ^}".format(
                vars(row) if not isinstance(row, dict) else row,
                id_width=id_width,
