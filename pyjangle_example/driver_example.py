...
async def main():
    ...
    response = await command_dispatcher(
        CreateAccount(name=account_names[1], initial_deposit=100)
    )
    assert response.is_success
    ...
    response = await command_dispatcher(
        DepositFunds(account_id=account_ids[0], amount=50.50)
    )
    event: FundsDeposited = await _dequeue_event()
    assert isinstance(event, FundsDeposited)
    assert int(event.account_id) == 1
    ...
    response = await command_dispatcher(
        WithdrawFunds(account_id=account_ids[1], amount=250)
    )
    assert not response.is_success
    assert response.data == "Insufficient funds"

    response = await command_dispatcher(
        WithdrawFunds(account_id=account_ids[1], amount=150)
    )
    assert response.is_success
    ...
    response = await command_dispatcher(
        Transfer(funded_account_id="None", funding_account_id=account_ids[0], amount=20)
    )
    assert response.is_success
...
async def receive_event(event: VersionedEvent):
    if has_registered_event_handler(type(event)):
        await default_event_dispatcher(event)
