...
async def saga_state_verifier(test_case: TestCase, fail_commands: list, exception_commands: list, preexisting_saga_events: list[VersionedEvent], incoming_event: VersionedEvent, saga_factory: Callable[[list[Event]], Saga], expected_saga_state: dict[str, any]):
            ...
            for k, v in expected_saga_state.items():
                ...
                if k == SAGA_FLD_NEW_EVENTS:
                    ...
                    test_case.assertFalse(
                        missing_from_expected, f"New events contains more than was expected: {str(missing_from_expected)}")
                    test_case.assertFalse(
                        extra_in_expected, f"New events was missing some expected items: {str(extra_in_expected)}")
                    continue
                if k == SAGA_FLD_FLAGS:
                    v = set(v)
                    missing_from_expected = actual.difference(v)
                    extra_in_expected = v.difference(actual)
                    test_case.assertFalse(
                        missing_from_expected, f"Flags contains more than was expected: {str(missing_from_expected)}")
                    test_case.assertFalse(
                        extra_in_expected, f"Flags was missing some expected items: {str(extra_in_expected)}")
...
class TestRequestFundsFromAnotherAccountSaga_ReceiveFundsRequestedEventReceived(IsolatedAsyncioTestCase):
    ...
    async def test_happy_path(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[], preexisting_saga_events=[], incoming_event=receive_funds_requested_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_TryObtainReceiveFundsApproval_exception(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[TryObtainReceiveFundsApproval], preexisting_saga_events=[], incoming_event=receive_funds_requested_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_NotifyReceiveFundsRejected_exception(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[TryObtainReceiveFundsApproval], exception_commands=[NotifyReceiveFundsRejected], preexisting_saga_events=[], incoming_event=receive_funds_requested_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_TryObtainReceiveFundsApproval_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[TryObtainReceiveFundsApproval], exception_commands=[], preexisting_saga_events=[], incoming_event=receive_funds_requested_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_NotifyReceiveFundsRejected_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[TryObtainReceiveFundsApproval, NotifyReceiveFundsRejected], exception_commands=[], preexisting_saga_events=[], incoming_event=receive_funds_requested_event, saga_factory=get_saga, expected_saga_state={
            SAGA_FLD_NEW_EVENTS: [ReceiveFundsRequested, TryObtainReceiveFundsApprovalCommandFailed, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_FLAGS: [ReceiveFundsRequested, TryObtainReceiveFundsApprovalCommandFailed, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_IS_COMPLETE: True,
            SAGA_FLD_IS_DIRTY: True,
...
class TestRequestFundsFromAnotherAccountSaga_ReceiveFundsApproved(IsolatedAsyncioTestCase):
    async def test_happy_path(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_DebitReceiveFundsCommandSucceeded_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[DebitReceiveFunds], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_DebitReceiveFundsCommandSucceeded_exception(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[DebitReceiveFunds], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_NotifyReceiveFundsRejectedCommandAcknowledged_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[DebitReceiveFunds, NotifyReceiveFundsRejected], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
            SAGA_FLD_NEW_EVENTS: [ReceiveFundsApproved, DebitReceiveFundsCommandFailed, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_FLAGS: [ReceiveFundsRequested, ReceiveFundsApproved, DebitReceiveFundsCommandFailed, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_IS_COMPLETE: True,
            SAGA_FLD_IS_DIRTY: True,
    ...
    async def test_CreditReceiveFunds_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[CreditReceiveFunds], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_RollbackReceiveFundsDebitCommandAcknowledged_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[CreditReceiveFunds, RollbackReceiveFundsDebit], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_approved_event, saga_factory=get_saga, expected_saga_state={
            SAGA_FLD_NEW_EVENTS: [ReceiveFundsApproved, DebitReceiveFundsCommandSucceeded, CreditReceiveFundsCommandFailed, RollbackReceiveFundsDebitCommandAcknowledged],
            SAGA_FLD_FLAGS: [ReceiveFundsRequested, ReceiveFundsApproved, DebitReceiveFundsCommandSucceeded, CreditReceiveFundsCommandFailed, RollbackReceiveFundsDebitCommandAcknowledged],
            SAGA_FLD_IS_COMPLETE: True,
            SAGA_FLD_IS_DIRTY: True,
...
class TestRequestFundsFromAnotherAccountSaga_ReceiveFundsRejected(IsolatedAsyncioTestCase):
    async def test_happy_path(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_rejected_event, saga_factory=get_saga, expected_saga_state={
    ...
    async def test_NotifyReceiveFundsRejected_failed(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[NotifyReceiveFundsRejected], exception_commands=[], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_rejected_event, saga_factory=get_saga, expected_saga_state={
            SAGA_FLD_NEW_EVENTS: [ReceiveFundsRejected, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_FLAGS: [ReceiveFundsRequested, ReceiveFundsRejected, NotifyReceiveFundsRejectedCommandAcknowledged],
            SAGA_FLD_IS_COMPLETE: True,
            SAGA_FLD_IS_DIRTY: True,
    ...
    async def test_NotifyReceiveFundsRejected_exception(self, *_):
        await saga_state_verifier(test_case=self, fail_commands=[], exception_commands=[NotifyReceiveFundsRejected], preexisting_saga_events=[receive_funds_requested_event], incoming_event=receive_funds_rejected_event, saga_factory=get_saga, expected_saga_state={
