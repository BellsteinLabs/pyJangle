...
class RequestFundsFromAnotherAccount(Saga):
    ...
    def on_receive_funds_requested(self, next_version: int):
        ...
        if request_transfer_approval_on_funding_account_command_response.is_success:
            self._post_new_event(TryObtainReceiveFundsApprovalCommandSucceeded(
                saga_id=self.saga_id, version=next_version))
            return
        else:
            try:
                self.command_dispatcher(
                    self._make_notify_receive_funds_rejected_command())
                self._post_new_event(NotifyReceiveFundsRejectedCommandSucceeded(
                    saga_id=self.saga_id, version=next_version))
                self.set_complete()
            except:
                self.set_retry(datetime.now + RETRY_TIMEOUT_LENGTH)
    ...
    def on_receive_funds_approved(self, next_version: int):
        if not DebitReceiveFunds in self.flags:
            ...
            if debit_transfer_funds_response.is_success:
                self._post_new_event(DebitReceiveFundsCommandSucceeded(
                    saga_id=self.saga_id, version=next_version))
            else:
                try:
                    self.command_dispatcher(
        ...
        if not CreditReceiveFunds in self.flags:
            ...
            if credit_receive_transfer_funds_response.is_success:
                self._post_new_event(CreditReceiveFundsCommandSucceeded(
                    saga_id=self.saga_id, version=next_version + 1 if DebitReceiveFunds in self.flags else next_version))
                self.set_complete()
            else:
                try:
                    self.command_dispatcher(
                        self._make_rollback_receive_funds_debit_command())
                    self._post_new_event(RollbackReceiveFundsDebitCommandSucceeded(
                        saga_id=self.saga_id, version=next_version + 1 if DebitReceiveFunds in self.flags else next_version))
                    self.set_complete()
                except:
                    self.set_retry(datetime.now + RETRY_TIMEOUT_LENGTH)
