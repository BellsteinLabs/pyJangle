...
from pyjangle_example.events import RequestApproved, RequestRejected, RequestCreated, TransferDebited
from pyjangle_example.saga import RequestSaga, SendFundsToAnotherAccountSaga


@register_event_handler(TransferDebited)
async def handle_send_funds_debited_event(event: TransferDebited):
    credit_command = CreditTransfer(funding_account_id=event.funding_account_id,
...
async def handle_receive_funds_requested(event: RequestCreated):
    await handle_saga_event(saga_id=event.transaction_id, saga_type=RequestSaga, event=event)
