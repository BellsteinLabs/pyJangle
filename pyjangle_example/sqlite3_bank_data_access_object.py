...
class Sqlite3BankDataAccessObject(BankDataAccessObject):
    ...
    async def handle_funds_deposited(event: FundsDeposited):
        ...
        transactions_q = make_update_transactions_query(event.account_id, TRANSACTION_TYPE.DEPOSIT, event)
    ...
    async def handle_funds_withdrawn(event: FundsWithdrawn):
        bank_summary_q = make_update_balance_query(event.account_id, event)
        transactions_q = make_update_transactions_query(event.account_id, TRANSACTION_TYPE.WITHDRAWAL, event)
    ...
    async def handle_receive_funds_debited(event: ReceiveFundsDebited):
        ...
        transactions_q = make_update_transactions_query(event.funding_account_id, TRANSACTION_TYPE.REQUEST_DEBIT)
    ...
    async def handle_receive_funds_debited_rolled_back(event: ReceiveFundsDebitedRolledBack):
        transfer_request_q = q_bldr(TABLES.TRANSFER_REQUESTS) \
            ...
            .done()
        bank_summary_q = make_update_balance_query(event.funding_account_id, event)
        transactions_q = make_update_transactions_query(event.funding_account_id, TRANSACTION_TYPE.REQUEST_DEBIT_ROLLBACK)
    ...
    async def handle_receive_funds_credited(event: ReceiveFundsCredited):
        ...
        transactions_q = make_update_transactions_query(event.funded_account_id, TRANSACTION_TYPE.REQUEST_CREDIT)
    ...
    async def handle_send_funds_credited(event: SendFundsCredited):
        transfer_q = q_bldr(TABLES.TRANSFERS) \
            ...
            .done()
        bank_summary_q = make_update_balance_query(event.funded_account_id, event)
        transactions_q = make_update_transactions_query(event.funded_account_id, TRANSACTION_TYPE.TRANSFER_CREDIT)
    ...
    async def handle_send_funds_debited(event: SendFundsDebited):
        transfer_q = q_bldr(TABLES.TRANSFERS) \
            ...
            .done()
        bank_summary_q = make_update_balance_query(event.funding_account_id, event)
        transactions_q = make_update_transactions_query(event.funding_account_id, TRANSACTION_TYPE.TRANSFER_DEBIT, event)
    ...
    async def handle_send_funds_debited_rolled_back(event: SendFundsDebitedRolledBack):
        transfer_q = q_bldr(TABLES.TRANSFERS) \
            ...
            .done()
        bank_summary_q = make_update_balance_query(event.funding_account_id, event)
        transactions_q = make_update_transactions_query(event.funding_account_id, TRANSACTION_TYPE.TRANSFER_DEBIT_ROLLBACK, event)
