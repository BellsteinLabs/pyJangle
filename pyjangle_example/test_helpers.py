...
THIRTY_MINUTES_FROM_FAKE_NOW = FAKE_CURRENT_TIME + datetime.timedelta(minutes=30)
NAME_FLD = "name"
...
def verify_events(
    test_case: TestCase,
    events: list[tuple[any, VersionedEvent]],
    ...
    if len(expected_events) != expected_length:
        raise Exception(
            f"""Expected_length '{expected_length}' and len(expected) 
            '{str(len(expected_events))}' must be equivalent"""
        )
    for i, expected_type, expected_aggregate_id, expected_version, expected_values in [
        (e.index, e.event_type, e.agg_id, e.version, e.attributes)
        ...
        if expected_type:
            test_case.assertIsInstance(
                actual_event,
                expected_type,
                f"""Expected type '{str(expected_type)}({str(i)})', but got type 
                '{str(type(actual_event))}'""",
            )
        if expected_aggregate_id:
            test_case.assertEqual(
                expected_aggregate_id,
                actual_aggregate_id,
                f"""Aggregate ID mismatch on event typed '{str(expected_type)}({str(i)})
                ': Expected '{expected_aggregate_id}', got '{actual_aggregate_id}'""",
            )
        if expected_version:
            test_case.assertEqual(
                expected_version,
                actual_event.version,
                f"""Version mismatch on event typed '{str(expected_type)}({str(i)})': 
                Expected '{expected_version}', got '{actual_event.version}'""",
            )
        if expected_values:
            for attribute_name, expected_value in expected_values.items():
                if not hasattr(actual_event, attribute_name):
                    raise Exception(
                        f"""Event of type '{str(expected_type)}({str(i)})' does not have 
                        an attribute named {attribute_name}"""
                    )
                actual_value = getattr(actual_event, attribute_name)
                test_case.assertEqual(
                    actual_value,
                    expected_value,
                    f"""Event typed '{str(expected_type)}({str(i)})': Attribute 
                    '{attribute_name}': Expected '{str(expected_value)}', got 
                    '{str(actual_value)}'""",
                )
