...
class TestSqlite3BankDataAccessObjectTest(IsolatedAsyncioTestCase):
    ...
    async def test_when_no_events_then_queries_are_empty(self, *_):
        bank_summary_response: list[AccountResponse] = await handle_query(AccountsList())
    ...
    async def test_when_account_created_event_then_account_queryable(self, *_):
        ...
        await expect_these_query_responses(
            ...
            bank_summary=[AccountResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), pending_request_count=0)],
            ...
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), transfer_requests=[]),
            account_ledger=[]
        )
    ...
    async def test_when_funds_deposited_then_funds_queryable(self, *_):
        ...
        await expect_these_query_responses(
            test_case=self,
            bank_summary=[AccountResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(AMOUNT), pending_request_count=0)],
            bank_stats=BankStatsResponse(active_accounts_count=1, deleted_accounts_count=0, net_account_balance=float(AMOUNT),
                                         transaction_count=1, average_deposit_amount=float(AMOUNT), average_withdrawal_amount=0, debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(AMOUNT), transfer_requests=[]),
            account_ledger=[TransactionResponse(
                initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="deposit")]
        )
    ...
    async def test_when_funds_withdrawn_then_funds_queryable(self, *_):
        ...
        await expect_these_query_responses(
            ...
            bank_summary=[AccountResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(-AMOUNT), pending_request_count=0)],
            bank_stats=BankStatsResponse(active_accounts_count=1, deleted_accounts_count=0, net_account_balance=float(-AMOUNT),
                                         transaction_count=1, average_deposit_amount=0, average_withdrawal_amount=float(AMOUNT), debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(-AMOUNT), transfer_requests=[]),
            account_ledger=[TransactionResponse(
                initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="withdrawal")]
        )
    ...
    async def test_when_debt_forgiven_then_balance_zero(self, *_):
        ...
        await expect_these_query_responses(
            ...
            bank_summary=[AccountResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), pending_request_count=0)],
            bank_stats=BankStatsResponse(active_accounts_count=1, deleted_accounts_count=0, net_account_balance=0,
                                         transaction_count=2, average_deposit_amount=0, average_withdrawal_amount=float(AMOUNT), debt_forgiven_amount=float(AMOUNT)),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), transfer_requests=[]),
            account_ledger=[
                TransactionResponse(
                    initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="withdrawal"),
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="debt_forgiveness")]
        )

    async def test_when_receive_funds_requested_then_nothing_really_happens(self, *_):
        account_created = AccountCreated(
            version=1, account_id=ACCOUNT_ID, name=NAME)
        receive_funds_requested = RequestCreated(version=2, funded_account_id=ACCOUNT_ID, funding_account_id=OTHER_ACCOUNT_ID,
    ...
    async def test_when_notified_receive_funds_requested_then_pending_request_added(self, *_):
        account_created = AccountCreated(
            version=1, account_id=ACCOUNT_ID, name=NAME)
        notified_receive_funds_requested = RequestReceived(version=2, funded_account_id=OTHER_ACCOUNT_ID, funding_account_id=ACCOUNT_ID,
                                                           amount=AMOUNT, transaction_id=TRANSACTION_ID_1, timeout_at=FAKE_CURRENT_TIME_PLUS)
        await publish_events(account_created, notified_receive_funds_requested)
        await expect_these_query_responses(
            test_case=self,
            bank_summary=[AccountResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), pending_request_count=1)],
            bank_stats=BankStatsResponse(active_accounts_count=1, deleted_accounts_count=0, net_account_balance=0,
                                         transaction_count=0, average_deposit_amount=0, average_withdrawal_amount=0, debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), transfer_requests=[TransferResponse(funded_account=OTHER_ACCOUNT_ID, amount=str(AMOUNT), state="request_received", timeout_at=FAKE_CURRENT_TIME_PLUS.isoformat(), transaction_id=TRANSACTION_ID_1)]),
            account_ledger=[]
        )
    ...
    async def test_when_notified_receive_funds_rejected_then_pending_request_removed(self, *_):
        ...
        receive_funds_requested = RequestCreated(version=2, funded_account_id=ACCOUNT_ID, funding_account_id=OTHER_ACCOUNT_ID,
                                                 amount=AMOUNT, transaction_id=TRANSACTION_ID_1, timeout_at=FAKE_CURRENT_TIME_PLUS)
        notified_receive_funds_requested = RequestReceived(version=2, funded_account_id=ACCOUNT_ID, funding_account_id=OTHER_ACCOUNT_ID,
                                                           amount=AMOUNT, transaction_id=TRANSACTION_ID_1, timeout_at=FAKE_CURRENT_TIME_PLUS)
        receive_funds_rejected = RequestRejected(
            version=3, funding_account_id=ACCOUNT_ID, transaction_id=TRANSACTION_ID_1)
        ...
        await expect_these_query_responses(
            ...
            bank_summary=[
                AccountResponse(account_id=OTHER_ACCOUNT_ID,
                                name=OTHER_NAME, balance=str(0), pending_request_count=0),
                AccountResponse(account_id=ACCOUNT_ID, name=NAME, balance=str(0), pending_request_count=0)],
    ...
    async def test_when_receive_funds_debited_then_transaction_added(self, *_):
        ...
        await expect_these_query_responses(
            ...
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(-AMOUNT), transfer_requests=[]),
            account_ledger=[TransactionResponse(
                initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="request_debit")]
        )
    ...
    async def test_when_receive_funds_debit_rolled_back_then_transaction_added(self, *_):
        ...
        receive_funds_debit_rolled_back = RequestDebitRolledBack(
            version=5, funding_account_id=ACCOUNT_ID, transaction_id=TRANSACTION_ID_1, amount=AMOUNT, balance=Decimal(0))
        ...
        await expect_these_query_responses(
            ...
            account_ledger=[
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME,
                                    amount=str(AMOUNT), transaction_type="request_debit"),
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="request_debit_rollback")]
        )
        ...
        await expect_these_query_responses(
            ...
            bank_summary=[
                AccountResponse(account_id=ACCOUNT_ID, name=NAME,
                                balance=str(AMOUNT), pending_request_count=0),
                AccountResponse(account_id=OTHER_ACCOUNT_ID, name=OTHER_NAME, balance=str(-AMOUNT), pending_request_count=0)],
            bank_stats=BankStatsResponse(active_accounts_count=2, deleted_accounts_count=0, net_account_balance=0,
                                         transaction_count=2, average_deposit_amount=0, average_withdrawal_amount=0, debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(AMOUNT), transfer_requests=[]),
            account_ledger=[
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="request_credit")]
        )
    ...
    async def test_when_transfer_debited_transaction_added(self, *_):
        ...
        other_account_created = AccountCreated(
            version=1, account_id=OTHER_ACCOUNT_ID, name=OTHER_NAME)
        transfer_debited = TransferDebited(version=2, funding_account_id=ACCOUNT_ID,
        ...
        await expect_these_query_responses(
            ...
            bank_stats=BankStatsResponse(active_accounts_count=2, deleted_accounts_count=0, net_account_balance=float(-AMOUNT),
                                         transaction_count=1, average_deposit_amount=0, average_withdrawal_amount=0, debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(-AMOUNT), transfer_requests=[]),
            account_ledger=[
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="transfer_debit")]
        )
    ...
    async def test_when_transfer_debit_rolled_back_transaction_added(self, *_):
        ...
        transfer_debit_rolled_back = TransferDebitRolledBack(
            version=3, amount=AMOUNT, funding_account_id=ACCOUNT_ID, balance=Decimal(0), transaction_id=TRANSACTION_ID_1)
        await publish_events(account_created, transfer_debited, transfer_debit_rolled_back)
        await expect_these_query_responses(
            test_case=self,
            bank_summary=[
                AccountResponse(account_id=ACCOUNT_ID, name=NAME,
                                balance=str(0), pending_request_count=0)],
            bank_stats=BankStatsResponse(active_accounts_count=1, deleted_accounts_count=0, net_account_balance=0,
                                         transaction_count=2, average_deposit_amount=0, average_withdrawal_amount=0, debt_forgiven_amount=0),
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(0), transfer_requests=[]),
            account_ledger=[
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME,
                                    amount=str(AMOUNT), transaction_type="transfer_debit"),
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME,
                                    amount=str(AMOUNT), transaction_type="transfer_debit_rollback"),
            ]
        )
    ...
    async def test_when_transfer_credit_transaction_added(self, *_):
        account_created = AccountCreated(
            version=1, account_id=ACCOUNT_ID, name=NAME)
        transfer_credited = TransferCredited(version=2, funding_account_id=OTHER_ACCOUNT_ID,
        ...
        await expect_these_query_responses(
            ...
            account_summary=AccountSummaryResponse(
                account_id=ACCOUNT_ID, name=NAME, balance=str(AMOUNT), transfer_requests=[]),
            account_ledger=[
                TransactionResponse(initiated_at=FAKE_CURRENT_TIME, amount=str(AMOUNT), transaction_type="transfer_credit")]
        )
