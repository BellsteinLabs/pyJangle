...
from pyjangle_example.commands import AcceptRequest, CreditRequest, CreditTransfer, DebitRequest, DeleteAccount, DepositFunds, NotifyRequestRejected, Request, RejectRequest, ForgiveDebt, RollbackRequestDebit, RollbackTransferDebit, Transfer, GetRequestApproval, WithdrawFunds
...
notified_receive_funds_requested_event_v2 = RequestReceived(version=2, funded_account_id=OTHER_ACCOUNT_ID,
                                                            funding_account_id=ACCOUNT_ID, amount=LARGE_AMOUNT, transaction_id=KNOWN_UUID, timeout_at=THIRTY_MINUTES_FROM_FAKE_NOW)
notified_receive_funds_requested_event_v3 = RequestReceived(version=3, funded_account_id=OTHER_ACCOUNT_ID,
                                                            funding_account_id=ACCOUNT_ID, amount=LARGE_AMOUNT, transaction_id=KNOWN_UUID, timeout_at=THIRTY_MINUTES_FROM_FAKE_NOW)
...
receive_funds_debited_event = RequestDebited(
    version=5, funding_account_id=ACCOUNT_ID, balance=Decimal(0), transaction_id=KNOWN_UUID, amount=LARGE_AMOUNT)

uuid_mock = Mock(wraps=uuid.uuid4, return_value=KNOWN_UUID)
# uuid_mock.uuid4 = Mock(return_value=KNOWN_UUID)
...
class TestAccountAggregate(IsolatedAsyncioTestCase):
    ...
    def test_when_send_funds_and_balance_insufficient_then_no_new_events_and_command_fails(self, *_):
        c = Transfer(funded_account_id=OTHER_ACCOUNT_ID,
    ...
    def test_when_send_funds_then_send_funds_debited(self, *_):
        ...
        verify_events(events=self.aggregate.new_events, test_case=self, expected_events=[
            ExpectedEvent(event_type=TransferDebited, agg_id=ACCOUNT_ID, version=3, attributes={
    ...
    def test_when_receive_funds_then_receive_funds_requested_event(self, *_):
        c = Request(funded_account_id=ACCOUNT_ID,
                    funding_account_id=OTHER_ACCOUNT_ID, amount=LARGE_AMOUNT)
        self.aggregate.apply_events([account_created_event])
        self.assertTrue(self.aggregate.validate(c).is_success)

        verify_events(events=self.aggregate.new_events, test_case=self, expected_events=[
            ExpectedEvent(RequestCreated, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
    ...
    def test_when_credit_send_funds_then_send_funds_credited_event(self, *_):
        ...
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(RequestCredited, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDED_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID,
    ...
    def test_when_try_obtain_receive_funds_approval_then_notified_receive_funds_request(self, *_):
        c = GetRequestApproval(funded_account_id=OTHER_ACCOUNT_ID, funding_account_id=ACCOUNT_ID,
                               transaction_id=KNOWN_UUID, timeout_at=THIRTY_MINUTES_FROM_FAKE_NOW, amount=LARGE_AMOUNT)
        self.aggregate.apply_events([account_created_event])
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestReceived, agg_id=ACCOUNT_ID, version=2, attributes={
                FUNDED_ACCOUNT_ID_FLD: OTHER_ACCOUNT_ID,
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                AMOUNT_FLD: LARGE_AMOUNT,
                TRANSACTION_ID_FLD: KNOWN_UUID,
    ...
    def test_when_reject_receive_funds_request_then_receive_funds_rejected_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(RequestRejected, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID
            })
        ])
    ...
    def test_when_accept_receive_funds_request_then_receive_funds_approved_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c))
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestApproved, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID
            })
        ])
    ...
    def test_when_notify_receive_funds_rejected_then_notified_receive_funds_rejected_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestRejectionReceived, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDED_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID
            })
        ])
    ...
    def test_when_debit_receive_funds_and_transaction_not_found_then_command_fails_and_no_new_events(self, *_):
        c = DebitRequest(funding_account_id=ACCOUNT_ID,
    ...
    def test_when_debit_receive_funds_then_receive_funds_debited_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestDebited, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                BALANCE_FLD: 0,
                TRANSACTION_ID_FLD: KNOWN_UUID,
                AMOUNT_FLD: LARGE_AMOUNT
    ...
    def test_when_credit_receive_funds_then_receive_funds_credited_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestCredited, agg_id=ACCOUNT_ID, version=3, attributes={
                FUNDED_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID,
                BALANCE_FLD: LARGE_AMOUNT,
                AMOUNT_FLD: LARGE_AMOUNT
    ...
    def test_when_credit_send_funds_then_send_funds_credited_event(self, *_):
        c = CreditTransfer(funding_account_id=OTHER_ACCOUNT_ID, funded_account_id=ACCOUNT_ID,
                           amount=SMALL_AMOUNT, transaction_id=KNOWN_UUID)
        self.aggregate.apply_events([account_created_event])
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=TransferCredited, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDED_ACCOUNT_ID_FLD: ACCOUNT_ID,
                FUNDING_ACCOUNT_ID_FLD: OTHER_ACCOUNT_ID,
                AMOUNT_FLD: SMALL_AMOUNT,
                BALANCE_FLD: SMALL_AMOUNT,
    ...
    def test_when_rollback_send_funds_debit_then_send_funds_debit_rolled_back_event(self, *_):
        ...
        send_funds_debited = TransferDebited(version=3, funded_account_id=OTHER_ACCOUNT_ID, funding_account_id=ACCOUNT_ID,
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=TransferDebitRolledBack, agg_id=ACCOUNT_ID, version=4, attributes={
                AMOUNT_FLD: SMALL_AMOUNT,
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                BALANCE_FLD: LARGE_AMOUNT,
                TRANSACTION_ID_FLD: KNOWN_UUID
    ...
    def test_when_rollback_receive_funds_debit_then_receive_funds_debit_rolled_back_event(self, *_):
        ...
        self.assertTrue(self.aggregate.validate(c).is_success)
        verify_events(test_case=self, events=self.aggregate.new_events, expected_events=[
            ExpectedEvent(event_type=RequestDebitRolledBack, agg_id=ACCOUNT_ID, version=self.aggregate.version + 1, attributes={
                FUNDING_ACCOUNT_ID_FLD: ACCOUNT_ID,
                TRANSACTION_ID_FLD: KNOWN_UUID,
                BALANCE_FLD: LARGE_AMOUNT,
                AMOUNT_FLD: LARGE_AMOUNT,
    ...
    def test_when_transaction_timed_out_then_command_fails(self, *_):
        notified_receive_funds_requested_event = RequestReceived(version=3, funded_account_id=OTHER_ACCOUNT_ID,
